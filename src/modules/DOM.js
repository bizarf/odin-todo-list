...
import { assignTaskId } from "./todos.js";
import { init } from "../index.js";

const taskLoader = (() => {
    // loads all the tasks in the array
    const allTasks = () => {
        const ul = document.querySelector("#project-allList");
    ...
    const todayTasks = () => {
        let ul = document.querySelector("#project-todayList");
        while (ul.firstChild) ul.removeChild(ul.firstChild);
        for (let task of taskStorage.tasks) {
    ...
    const weekTasks = () => {
        let ul = document.querySelector("#project-weekList");
        while (ul.firstChild) ul.removeChild(ul.firstChild);
        for (let task of taskStorage.tasks) {
    ...
    const listDom = (list, task) => {
        ...
        if (task.isComplete === true) {
            titleDiv.classList = "listItem taskCompleted";
        } else {
...
const projectLoader = (() => {
    ...
    const projectPageCreation = () => {
        ...
        for (let project of projectStorage.projects) {
            ...
            ul.id = `project-${project.projectTitle}List`;
            div.appendChild(h2);
            div.appendChild(hr);
            div.appendChild(ul);
    ...
    const projectTasks = () => {
        for (let project of projectStorage.projects) {
            let ul = document.querySelector(
                `#project-${project.projectTitle}List`
            );
            while (ul.firstChild) ul.removeChild(ul.firstChild);
            if (taskStorage.tasks === null) {
                taskStorage.tasks = [];
    ...
    projectDeleteBtn.forEach((button) => {
        button.addEventListener("click", () => {
            ...
            for (let i = taskStorage.tasks.length - 1; i >= 0; i--) {
                if (
                    taskStorage.tasks[i].projectTitle ===
...
export { taskLoader, taskFunctions, projectLoader, projectDelete };
